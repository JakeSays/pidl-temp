...
//lifted from https://github.com/dart-lang/graphs
...
List<T> topologicalSort<T>(List<T> nodes, Iterable<T> Function(T) edges,
    ...
    Comparator<T>? secondarySort}) 
{
    ...
    void visit(T node) 
    {
        ...
        final edgeNodes = edges(node).toList();
        temporaryMark.add(node);
        for (var child in edgeNodes) 
        {
            visit(child);
        }
    ...
    int Function(T)? hashCode) 
{
    ...
    if (result.length < nodes.length) 
    {
        ...
        topologicalSort<T>(nodes.where(nodeIsInCycle).toList(), edges,
